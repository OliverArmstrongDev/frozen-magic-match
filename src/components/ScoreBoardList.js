...
export default function ScoreBoardList() {
  ...
  const { documents, setDocuments, error } = useCollection(
    "GameResults",
  ...
  useEffect(() => {
    if (user && documents && documents.lastscore > 0) {
      console.log("useeffect", documents.lastscore);

      dispatch({
        type: actions.LAST_SCORE,
        payload: documents[documents.length - 1].lastScore,
      });
    }
    ...
    <div>
      ...
      <div className="scorelist-container">
        ...
        {(!user || !documents || !documents.length) && (
          <h4>Play a game and save your score to see results here</h4>
        )}
        {documents &&
          documents.map((doc) => (
